 ...
 */
function start() {
    var nextmap1 = cm.getClient().getChannelServer().getMapFactory().getMap(108010201);
    var nextmap2 = cm.getClient().getChannelServer().getMapFactory().getMap(108010301);
    var nextmap3 = cm.getClient().getChannelServer().getMapFactory().getMap(108010101);
    var nextmap4 = cm.getClient().getChannelServer().getMapFactory().getMap(108010401);
    var nextmap5 = cm.getClient().getChannelServer().getMapFactory().getMap(108010501);
    var nextmap11 = cm.getClient().getChannelServer().getMapFactory().getMap(108010200);
    var nextmap22 = cm.getClient().getChannelServer().getMapFactory().getMap(108010300);
    var nextmap33 = cm.getClient().getChannelServer().getMapFactory().getMap(108010100);
    var nextmap44 = cm.getClient().getChannelServer().getMapFactory().getMap(108010400);
    var nextmap55 = cm.getClient().getChannelServer().getMapFactory().getMap(108010500);
		...
		if (cm.canHold(4031059)) {
			if (!(cm.haveItem(4031059))) {
				...
				if (cm.getPlayer().getMapId() == 100040106 && nextmap11.playerCount() == 0 && nextmap1.playerCount() == 0 && cm.getJob() == 210 || cm.getJob() == 220 || cm.getJob() == 230) {
					...
					cm.sendOk("通往异界的门,你可知道里面是多么的可怕!");
				}
			} else {
				cm.sendOk("你已经有了#t4031059#。");
			}
		} else {
			cm.sendOk("请确认是否有足够的背包空间");
		}
	} else {
		cm.sendOk("您的等级不符合要求!");
	}
