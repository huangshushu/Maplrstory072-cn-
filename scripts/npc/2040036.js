...
function action(mode, type, selection) {
	...
	if (cm.isLeader()) { // Leader
		...
		if (cm.haveItem(4001022, 25)) { // Clear stage
		    cm.sendNext("恭喜！你已经通过了第一阶段。快点现在，去第二阶段。");
		...
		cm.sendOk("欢迎来到第一阶段。#b废弃的塔#k 请收集#r#t4001022##k 来找我即可完成任务。");
		eim.setProperty("stage1leader","done");
		cm.dispose();
	    }
	} else { // Members
	    cm.sendNext("欢迎来到第一阶段。#b废弃的塔#k 请收集#r#t4001022##k 给你的队长，然后叫队长来找我即可完成任务。");
	    cm.dispose();
	}
    } else {
	cm.sendNext("恭喜！你已经通过了第一阶段。快点现在，去第二阶段。");
