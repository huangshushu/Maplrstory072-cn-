...
function action(mode, type, selection) {
	...
	if (eim.getProperty("crackLeaderPreamble") == null) {
	    ...
	    cm.sendNext("请干掉窗台上的 #b黑色老鼠#k 然后就会召唤 #b阿丽莎乐#k杀掉之后捡到钥匙再来找我。");
	    cm.dispose();
	} else {
	    if (cm.haveItem(4001023)) {
		status = 0;
		cm.sendNext("恭喜完成，想要前往奖励之地？");
	    } else {
		cm.sendNext("请打败#r阿丽莎乐#k 给我#t4001023#。");
		cm.dispose();
	    }
	}
    } else if (status == -1 && !cm.isLeader()) {
	cm.sendNext("请干掉窗台上的 #b黑色老鼠#k 然后就会召唤 #b阿丽莎乐#k杀掉之后捡到钥匙再来来请队长找我。");
	cm.dispose();
