...
function action(mode, type, selection) {
    if (mode == 1) {
        ...
        if (status > 2) {
            if (section == 0) {
                cm.sendOk("请慎重考虑。一旦你做出了決定，让我知道。");
            } else {
                cm.sendOk("想想吧，然后让我知道你的決定。");
            }
            cm.safeDispose();
        }
        status--;
    ...
    if (status == 0) {
        ...
        if (section == 0) {
			...
			if (cm.haveItem(4031348)) {
				...
				cm.sendNext("请到玩具塔44层找NPC购买#t4031348#。");
				cm.safeDispose();
			}
			} else {
				cm.sendNext("请检查你的背包是否已满。");
				cm.safeDispose();
			}
        } else {
