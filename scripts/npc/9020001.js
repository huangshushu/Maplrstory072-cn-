...
var prizeIdScroll = Array(2044901,2044902,2044802,2044801,2044702,2044701,2044602,
                         ...
                         2040514,2040513,2040502,2040501,2040323,2040321,2040317,2040316,2040302,2040301);						// Helmet, Topwear and Bottomwear for DEF
    ...
    if (curMap == 1) { // First Stage.
        if (playerStatus) { // Check if player is leader
            if (status == 0) {
                ...
                if (preamble == null) {
                    cm.sendNext("嗨，我是#p9020001# 欢迎来到废弃都市组队训练场，我需要队员的通行证，请叫队员收集完成后把通行证收集起来然后给我。");
            ...
            if (status == 0 && preamble == null) {
                ...
                cm.sendNext("在这里，你需要收集 #b通行证#k 过击败鳄鱼的数目相同的答案为单独提出的问题。");
...
//		    cm.sendNext("Please hurry on to the next stage, the portal opened!");
                    ...
                    if (totplayers == 3 || debug) {
                        ...
                        if (debug) {
                            ...
                            cm.sendNext("看起来你还没找到 3 " + nthobj + " 只是还没有。请想一个不同的组合 " + nthobj + ". 只有3被允许" + nthverb + " 在 " + nthobj + ", 如果你 " + nthpos + " 它可能不算是一个答案，所以请记住这一。继续前进!");
                        }
                        cm.dispose();
                    }
                }
