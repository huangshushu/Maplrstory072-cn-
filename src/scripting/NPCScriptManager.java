...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    public final void start(final MapleClient c, final int npc) {
        ...
        try {
            ...
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                if (iv == null) {
                    ...
                    if (iv == null) {
                        if (iv == null) {
                            if (c.getPlayer().isAdmin()) {
                                c.getPlayer().dropMessage(6, "找不到NPC脚本(ID:" + npc + "),所在地图(ID:" + c.getPlayer().getMapId() + ")");
                            }
                            System.out.println("找不到NPC脚本(ID:" + npc + "),所在地图(ID:" + c.getPlayer().getMapId() + ")");
                        }
                        dispose(c);
                        return;
                    }
            ...
            System.err.println("NPC脚本出错（ID : " + npc  + " \r\n错误内容: " + e);
            FileoutputUtil.log("logs/脚本异常.txt", "NPC脚本出错（ID : " + npc  + ".\r\n错误信息：" + e);
            dispose(c);
        } finally {
            lock.unlock();
        }
