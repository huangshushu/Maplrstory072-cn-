...
public abstract class AbstractPlayerInteraction {
    ...
    public final int haveItemQuantity(final int itemid) {
        return haveItemQuantity(itemid, false, true);
    }

    public final int haveItemQuantity(final int itemid, final boolean checkEquipped, final boolean greaterOrEquals) {
        return c.getPlayer().haveItemQuantity(itemid, checkEquipped, greaterOrEquals);
    }
    ...
    public final void gainLmoExp_PQ() {
        final int amount = (((int) Math.floor((getPlayer().getLevel() - 71) / 10)) * 10000 + 350000);
        gainExp(amount * c.getChannelServer().getExpRate());
    }

    public final void gainDwExp_PQ() {
        final int amount = (((int) Math.floor((getPlayer().getLevel() - 45) / 10)) * 5000 + 80000);
        gainExp(amount * c.getChannelServer().getExpRate());
    }

    public final void gainHdExp_PQ() {
        final int amount = (((int) Math.floor((getPlayer().getLevel() - 55) / 10)) * 7000 + 100000);
        gainExp(amount * c.getChannelServer().getExpRate());
    }
